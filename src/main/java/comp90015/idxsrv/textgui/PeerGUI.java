		...
		public void onInput(Window basePane, KeyStroke keyStroke, AtomicBoolean deliverEvent) {
			if(sharingTable.isFocused()) {
				if(keyStroke.getKeyType()==KeyType.Character) {
					//make dropShareWithIdxServer operation in PeerGUI, click 'd' to trigger it
					if(keyStroke.getCharacter()=='d') {
						...
						//call the corresponding interface in IPeer and get the return value 
						boolean dropped=peer.dropShareWithIdxServer(relativeName, shareRecord);
						//if drop successfully
						if(dropped) {
				...
				if(keyStroke.getKeyType()==KeyType.Character) {
					if(keyStroke.getCharacter()=='d') {
						...
						//call download interface in IPeer
						try {
