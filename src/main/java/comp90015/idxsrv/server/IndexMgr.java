...
public class IndexMgr {
	...
	public RETCODE share(String ip, 
		...
		//怎麽drop的？？？？？？ -> 在drop()中match成功的話，就會直接drop掉
	...
	public RETCODE drop(String ip,
		...
		if(sharerMap.containsKey(sharerKey)) {
			...
			//if provided secret does not match, return FIALEDSECRET code
	...
	public ArrayList<IndexElement> search(String[] keywords, int maxhits) {
		...
		//tranverse through all the files
		for(IndexElement element : sharerMap.values()) {
			if(md5s.contains(element.fileDescr.getFileMd5())) {
				continue;
			}
			boolean hit=true;
			//tranverse through all keywords
			//once the file that in this loop contains all the keywords, then the hitting succeeds
